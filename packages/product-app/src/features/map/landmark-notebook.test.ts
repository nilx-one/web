...
describe("the landmark notebook", () => {
  ...
  it("sends an Avaia to the nearest thing it has not studied, within reach", () => {
    ...
    expect(nextLandmarkToStudy(notebook, "0skai", from)?.id).toBe("poi:near");

    const studied = studyLandmark(notebook, near, "0skai", 2);
    expect(nextLandmarkToStudy(studied, "0skai", from)?.id).toBe("poi:far");
    // Another Avaia has its own notes.
    expect(nextLandmarkToStudy(studied, "0other", from)?.id).toBe("poi:near");

    const done = studyLandmark(studied, far, "0skai", 3);
    // What is out of reach stays where it is.
    expect(nextLandmarkToStudy(done, "0skai", from)).toBeUndefined();
    expect(studiedBy(done, "0skai").map((entry) => entry.landmark.id)).toEqual([
      "poi:far",
      "poi:near",
    ]);
  });
  ...
  it("only goes where its owner has been", () => {
    expect(nextLandmarkToStudy(EMPTY_NOTEBOOK, "0skai", from)).toBeUndefined();
  });
