...
The canonical production Web origin is `https://app.nilx.one`. Telegram Mini App authentication remains an ephemeral host envelope under `/telegram/`. Browser authentication starts at `/auth?provider=telegram`, returns through `/auth/callback?provider=telegram`, and becomes a server-managed same-origin session before the shared identity API accepts it. The auth router accepts only explicitly implemented provider values and fails closed for a missing, repeated, or unknown `provider`. Both Telegram evidence forms must resolve to the same Telegram user identifier and therefore the same Stage 1 Bond identity; a browser login must never create a second provider namespace or identity record.
