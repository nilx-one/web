...
palette. The shared `rig.py` captures loft sections and sleeve paths while that
...
The runtime serves `/avatars/0.1.0/{modelId}.glb` from its own static origin,
...
identical cross-model rotation tracks and clip endpoints. These checks do not
