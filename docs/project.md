...
This document is the durable README source for the GitHub Project that tracks work in [`nilx-one/web`](https://github.com/nilx-one/web). The Project coordinates delivery; it does not define protocol truth.
...
Repository and Project work must preserve the architecture documented by the codebase:
...
Work should remain small enough to review as one coherent task:
...
An issue should describe the intended outcome, the owning architectural boundary, acceptance criteria, and the evidence required to consider the work complete. Pull requests should link back to the issue or Project item they implement.
