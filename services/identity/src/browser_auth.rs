...
use crate::{
    IdentityProvider, IdentityRecord, IdentityRepository, NativeAuthConfig, ProviderBindingOutcome,
    ProviderIdentity, PubDress, SecretDigester, TokenFactory,
};
...
pub fn router(
    ...
    Router::new()
        .route(
            "/api/v1/auth/browser/telegram/start",
            get(start_telegram),
        )
        .route(
        ...
        .route(
            "/api/v1/auth/browser/discord/start",
            get(start_discord),
        )
        .route(
...
fn start_provider(
    ...
    let Some(state_token) = random_urlsafe(24) else {
        return service_unavailable();
    };
    let Some(code_verifier) = random_urlsafe(32) else {
        return service_unavailable();
    };
    ...
    };
    {
        ...
        match provider {
            BrowserProvider::Telegram => {
                query.append_pair("scope", "openid profile");
            }
            BrowserProvider::Discord => {
                query.append_pair("scope", "identify");
            }
        }
    }
...
async fn telegram_callback(
        ...
        .await
    {
        Ok(response) if response.status().is_success() => {
            match response.json::<TelegramTokenResponse>().await {
                Ok(value) => value,
                Err(error) => {
...
async fn discord_callback(
    ...
    let user = match state
        ...
        .await
    {
        Ok(response) if response.status().is_success() => match response.json::<DiscordUser>().await {
            Ok(value) => value,
            Err(error) => {
...
async fn finish_provider_callback(
    ...
    let now = match now_unix_seconds() {
        Some(value) => value,
        None => return callback_failure("provider_authentication_unavailable"),
    };
    ...
    if let Some(identity) = native_session_identity(state, headers, now).await {
        ...
        match state.repository.bind_provider(&pub_dress, &provider).await {
            Ok(ProviderBindingOutcome::Bound | ProviderBindingOutcome::AlreadyBound) => {
            ...
            Ok(ProviderBindingOutcome::ProviderAlreadyBound) => {
                return callback_failure("provider_already_linked");
            }
            Ok(ProviderBindingOutcome::IdentityMissing) => {
                return callback_failure("provider_authentication_unavailable");
            }
            Err(error) => {
...
async fn read_provider_context(
    ...
    let pending = read_cookie(&headers, PENDING_PROVIDER_COOKIE).and_then(|value| {
        state
            .cookie_signer
    ...
    if read_cookie(&headers, PENDING_PROVIDER_COOKIE).is_some() && pending.is_none() {
...
async fn link_pending_provider(
    ...
    match state.repository.bind_provider(&pub_dress, &provider).await {
        ...
        Ok(ProviderBindingOutcome::ProviderAlreadyBound) => no_store_error(
            StatusCode::CONFLICT,
            "provider_already_linked",
            "That provider account is already linked to another Bond.",
        ),
        Ok(ProviderBindingOutcome::IdentityMissing) => no_store_error(
            StatusCode::UNAUTHORIZED,
            "native_authentication_required",
            "Sign in to the Bond before linking this provider.",
        ),
...
mod tests {
    use super::{
        BrowserOAuthConfig, BrowserProvider, OAuthClientCredentials, PendingProvider, SignedCookie,
    };
    ...
    fn callbacks_are_pinned_to_the_public_origin() {
        let config = BrowserOAuthConfig::new(
            ...
            Some(OAuthClientCredentials::new("telegram", "secret")),
            Some(OAuthClientCredentials::new("discord", "secret")),
        );
        assert_eq!(
